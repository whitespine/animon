...
export class SystemChatMessage<SubType extends ChatMessage.SubType = ChatMessage.SubType> extends ChatMessage<SubType> {
    // State
    _svelte_wrappers: Record<string, HTMLLIElement> = {}; // For popout, popup, and 
    _svelte_components: Record<string, ReturnType<typeof mount>> = {};

    // Populate this as the specified component. Cannot be changed once populated
    async populateAsComponent(mode: string) {
        if (!this._svelte_wrappers[mode]) {
            ...
            wrapper.dataset["messageId"] = this.id ?? undefined;
            this._svelte_components[mode] = mount(SystemMessageComponent, {
                props: {
                    message: this
                }, target: wrapper
            });
    ...
    async renderHTML(options: Parameters<ChatMessage["renderHTML"]>[0] = {}) {
