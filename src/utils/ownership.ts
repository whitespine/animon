 ...
 */
export function onlineOwners(document: foundry.abstract.Document<any, any, any>, level = CONST.DOCUMENT_OWNERSHIP_LEVELS.OWNER) {
 ...
 * @returns 
 */
export function owners(document: foundry.abstract.Document<any, any, any>, level = CONST.DOCUMENT_OWNERSHIP_LEVELS.OWNER) {
    let result = [];
    for (let user of game.users.contents) {
        if (user.isGM || document.getUserLevel(user) >= level) {
            result.push(user);
        }
    }
    return result;
