...
export class ControlledLengthArrayField extends fields.ArrayField {
  ...
  constructor(element: Field, options: B & {length: number}) {
...
// For sorts
export class SortField extends fields.NumberField<foundry.data.fields.NumberField.Options & {initial: number}, number, number, number> {
  constructor() {
    super({
      initial: 0, 
      integer: true
 ...
 */
export function rankedSort<T>(array: T[], ranker: Ranker<T>): T[] {
  // Compute ranker for all elements
  let ir: Array<[T, Array<number | string>]> = array.map(x => [x, ranker(x)]);

  // Perform a sort
  ir.sort(([a, a_rank], [b, b_rank]) => {
    for(let i=0; i<a_rank.length && i<b_rank.length; i++) {
      if(typeof a_rank[i] == "string" && typeof b_rank[i] == "string") {
        let c = (a_rank[i] as string).localeCompare(b_rank[i] as string);
        if(c) return c;
      } else if(typeof a_rank[i] == "number" && typeof b_rank[i] == "number") {
        let c = (a_rank[i] as number) - (b_rank[i] as number);
        if(c) return c;
      } else {
